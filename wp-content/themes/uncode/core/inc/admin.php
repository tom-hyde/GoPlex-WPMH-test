...
function uncode_list_images() {
  ...
  wp_send_json_success(
  	array(
      'message' => sprintf( esc_html__( 'The Adaptive Images system is using %1$s of the %2$s space left.', 'uncode' ), $bytes, formatBytes(disk_free_space("."), 2) )
  ) );
}
...
if ( $max_input_vars < uncode_get_recommended_max_input_vars() ) {
	...
	if (is_admin() && $pagenow === 'admin.php' && isset( $_GET['page'] ) && $_GET['page'] === 'uncode-options') {
	    ...
	    <div class="notice error is-dismissible">
	      ...
	      <p>
	      	<?php echo sprintf( wp_kses(__( 'Before saving the theme options you need to address an issue marked on the <strong><a href="%s">Welcome Screen</a></strong>.', 'uncode' ), array( 'strong' => array(), 'a' => array( 'href' => array(),'target' => array() ) ) ), admin_url('admin.php?page=uncode-system-status') ); ?>
	      </p>
	    </div>
	    <?php
		}
...
function uncode_check_if_theme_was_updated() {
	if ( ! get_option( 'uncode_latest_version' ) || version_compare( get_option( 'uncode_latest_version' ), UNCODE_PARENT_VERSION, '<' ) ) {
...
function uncode_related_post_submenu() {
	if ( class_exists( 'RP4WP' ) && class_exists('RP4WP_Hook_Settings_Page') ) {
		...
		add_submenu_page( 'uncode-system-status', 'UNCODE', esc_html__( 'Related Posts', 'uncode' ), 'manage_options', 'rp4wp', array(
